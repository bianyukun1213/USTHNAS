...
logger.Debug("程序启动。");
Console.WriteLine($"{appTitle} 由 Hollis 编写，源代码、许可证、版本更新及项目说明见 https://github.com/bianyukun1213/USTHNAS。");

DateTimeOffset curDateTime = DateTimeOffset.Now;
List<SignInTask> tasks = [];
Dictionary<string, int> retries = [];
Dictionary<string, StatusCache> accountStatusCache = [];
...
try
{
    ...
    else
    {
        ...
        var cache = JsonNode.Parse(cacheStr)!.Deserialize<Dictionary<string, StatusCache>>();
        if (cache != null)
            accountStatusCache = cache;
    }
}
catch (Exception ex)
{
...
try
{
    ...
    foreach (SignInConfig conf in Config.SignInConfigs)
    {
        ...
        if (string.IsNullOrEmpty(conf.LoginAccount?.Trim()))
        {
        ...
        if (string.IsNullOrEmpty(conf.CollegeId?.Trim()))
        {
            logger.Warn(getSignInConfigSkippedStr("学校 Id 为空"));
            continue;
        }
...
{
    if (!accountStatusCache.TryAdd(task.LoginAccount, new StatusCache { Account = task.Account, JWT = task.JWT, LastSuccess = task.LastSuccess }))
        accountStatusCache[task.LoginAccount] = new StatusCache { Account = task.Account, JWT = task.JWT, LastSuccess = task.LastSuccess };
    UpdateStatus();
    RunNextTask();
}
...
try
{
    File.WriteAllText(cachePath, JsonSerializer.Serialize(accountStatusCache, ServiceOptions.jsonSerializerOptions));
}
catch (Exception ex)
{
